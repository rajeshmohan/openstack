...
class Firewall_db_mixin(firewall.FirewallPluginBase, base_db.CommonDbMixin,
    ...
    def _make_firewall_dict(self, fw, fields=None):
        ...
        if fw.get('service_context_id'):
            ...
            svc_ctxt_dict = {}
            for ctxt_type in constants.SI_SUPPORTED_RESOURCE_TYPES:
                db_entries = svc_ctxt[ctxt_type] or []
                svc_ctxt_dict[ctxt_type] = []
                for db_entry in db_entries:
                    c_id = '%s_id' % ctxt_type[:-1]
                    svc_ctxt_dict[ctxt_type].append(db_entry.get(c_id))
    ...
    def _process_service_context_for_fw(self, context, firewall):
        ...
        return self._process_service_context(context, fw_data)
