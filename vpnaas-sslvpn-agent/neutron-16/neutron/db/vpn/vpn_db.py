...
class VPNCredential(model_base.BASEV2, models_v2.HasId, models_v2.HasTenant):
    __tablename__ = "vpncredentials"
    name = sa.Column(sa.String(255))
    ...
    crl = sa.Column(sa.Text())


class SSLVPNConnection(model_base.BASEV2,
                       models_v2.HasId, models_v2.HasTenant):
    __tablename__ = "ssl_vpn_connections"
    name = sa.Column(sa.String(255))
    credential_id = sa.Column(sa.String(36),
                              sa.ForeignKey('vpncredentials.id'))
    admin_state_up = sa.Column(sa.Boolean())
    vpnservice_id = sa.Column(sa.String(36),
                              sa.ForeignKey('vpnservices.id'))
    client_address_pool_cidr = sa.Column(sa.String(64), nullable=False)
    status = sa.Column(sa.String(16), nullable=False)
    vpn_credential = orm.relationship(
        VPNCredential, backref='ssl_vpn_connections')
...
class VPNService(model_base.BASEV2, models_v2.HasId, models_v2.HasTenant):
    ...
    ssl_vpn_connections = orm.relationship(
        SSLVPNConnection,
        backref='vpnservice',
        cascade="all, delete-orphan")
...
class VPNPluginRpcDbMixin():
    ...
    def _update_vpn_status_by_agent(self, context, service_status_info_list):
