...
from neutron.tests.unit.db.vpn import test_db_sslvpn
...
class TestIPsecVPNDriverPlugin(test_db_vpnaas.TestVpnaas,
    ...
    def setUp(self):
        ...
        self.driver.service_type = vpn_consts.IPSEC
        driver_cls.return_value = self.driver
        super(TestIPsecVPNDriverPlugin, self).setUp(
            vpnaas_plugin=VPN_DRIVER_CLASS)

    def test_create_ipsec_site_connection(self, **extras):
        super(
            TestIPsecVPNDriverPlugin, self).test_create_ipsec_site_connection()
    ...
    def test_delete_vpnservice(self, **extras):
        super(TestIPsecVPNDriverPlugin, self).test_delete_vpnservice()
        self.driver.delete_vpnservice.assert_called_once_with(
            mock.ANY, mock.ANY)

    @contextlib.contextmanager
    def ipsec_vpnservice_set(self):
    ...
    def test_get_agent_hosting_vpn_services(self):
        with self.ipsec_vpnservice_set():
    ...
    def test_update_status(self):
        with self.ipsec_vpnservice_set() as vpnservice:
...
class TestSSLVPNDriverPlugin(test_db_sslvpn.TestSSLVPN,
                             test_agent_scheduler.AgentSchedulerTestMixIn,
                             test_agent_ext_plugin.AgentDBTestMixIn):
    def setUp(self):
        self.addCleanup(mock.patch.stopall)
        ...
        driver_cls_p = mock.patch(
            'neutron.services.vpn.'
            'service_drivers.sslvpn.SSLVPNDriver')
        driver_cls = driver_cls_p.start()
        self.driver = mock.Mock()
        self.driver.service_type = vpn_consts.SSL
        driver_cls.return_value = self.driver
        super(TestSSLVPNDriverPlugin, self).setUp(
            vpnaas_plugin=VPN_DRIVER_CLASS)

    def test_create_ssl_vpn_connection(self, **extras):
        super(
            TestSSLVPNDriverPlugin, self).test_create_ssl_vpn_connection()
        self.driver.create_ssl_vpn_connection.assert_called_once_with(
            mock.ANY, mock.ANY)
        self.driver.delete_ssl_vpn_connection.assert_called_once_with(
            mock.ANY, mock.ANY)

    @contextlib.contextmanager
    def ssl_vpnservice_set(self):
        """Test case to create a ssl_vpn_connection."""
        with self.subnet() as subnet:
            with self.router() as router:
                plugin = manager.NeutronManager.get_plugin()
                agent = {'host': FAKE_HOST,
                ...
                with self.vpnservice(
                    router=router, subnet=subnet) as vpnservice1:
                    with self.ssl_vpn_connection(
                        vpnservice=vpnservice1):
                        yield vpnservice1['vpnservice']

    def _agent_hosting_vpn_services(self):
        with self.ssl_vpnservice_set():
            service_plugin = manager.NeutronManager.get_service_plugins()[
                p_constants.VPN]
            vpnservices = service_plugin._get_agent_hosting_sslvpn_services(
                self.adminContext, FAKE_HOST)
            vpnservices = vpnservices.all()
            self.assertEqual(1, len(vpnservices))
            vpnservice_db = vpnservices[0]
            self.assertEqual(1, len(vpnservice_db.ssl_vpn_connections))
            ssl_vpn_connection = vpnservice_db.ssl_vpn_connections[0]
            self.assertIsNotNone(
                ssl_vpn_connection['vpn_credential'])

    def test_update_status(self):
        with self.ssl_vpnservice_set() as vpnservice:
            self._register_agent_states()
            service_plugin = manager.NeutronManager.get_service_plugins()[
                p_constants.VPN]
            service_plugin.update_status_by_agent(
